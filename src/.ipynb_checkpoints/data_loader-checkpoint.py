...
def poison_data(df, percentage):
    """
    Poisons a percentage of the dataset with random values.
    """
    if percentage == 0:
        return df
    ...
    # Get the feature columns to poison
    feature_cols = ['sepal length (cm)', 'sepal width (cm)',
                   'petal length (cm)', 'petal width (cm)']

    for idx in poisoned_indices:
        for col in feature_cols:
            # Replace with random value between min and max of the column
            min_val = df[col].min()
            max_val = df[col].max()
...
if __name__ == "__main__":
    df = load_iris_data()
    validate_data(df)
    # Example of poisoning 10% of the data
    poisoned_df = poison_data(df.copy(), 0.10)
    print("\nPoisoned Data Head:")
    print(poisoned_df.head())